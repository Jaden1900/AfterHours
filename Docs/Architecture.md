# AfterHours Architecture

## Foundation

AfterHours is a Unity 6 Universal Render Pipeline (URP) project written in C#. The project contains focused player, interaction, NPC, and basic dialogue systems without custom assembly definitions.

## Asset layout

All new game-owned assets belong in `Assets/AfterHours`:

- `Art`, `Audio`, `Content`, `Prefabs`, `Scenes`, and `Settings` hold authored Unity assets by type.
- `Scripts` holds runtime and editor C# source.
- `Tests` holds Unity Test Framework tests once behavior exists to test.

`Scripts` is organized by feature and responsibility. `Core` contains shared, dependency-light building blocks; feature folders (such as `NPCs`, `Dialogue`, `Quests`, and `AI`) own their domain code. Editor-only scripts belong in `Scripts/Editor`.

## Dependencies

Keep dependencies directed toward `Core`. A feature may use `Core`, but `Core` must not depend on gameplay features. Features should communicate through small interfaces, data objects, or events rather than referencing one another's concrete controllers.

No `.asmdef` files are included yet. With no custom production code, Unity's default assembly is the simplest correct option. Introduce assembly definitions only when a concrete module needs faster compile isolation or a well-defined dependency boundary; document references at that time and keep the graph acyclic.

## Scenes

The source-controlled scene set is intentionally minimal:

- `00_Bootstrap`: future application initialization entry point.
- `01_MainMenu`: future menu entry point.
- `90_Sandbox`: isolated general-purpose experimentation.
- `91_AIConversationTest`: isolated AI conversation integration testing.
- `92_QuestTest`: isolated quest-flow testing.

These are placeholder scenes only. They must not become the playable district.


## NPC and dialogue boundary

`Gameplay/NPCs/NPCData` holds authored NPC identity and one initial dialogue line without scene dependencies. `NPCInteractable` implements the shared interaction contract and receives its `NPCData` and `DialogueController` through explicit serialized references. `Gameplay/Dialogue/DialogueController` owns active dialogue state, supports a sequence of pages, and emits state-change events. `UI/DialoguePresenter` is the concrete TMP/button presentation layer and never owns dialogue state. `DialoguePlayerControlLock` disables the existing movement and camera controller while dialogue is open, restoring both when it closes. `DialogueCancelInputHandler` bridges the existing input reader's consumed Cancel action to the controller. This keeps dialogue state independent from UI and leaves a clear extension point for scripted lines, branching, quests, and AI conversations.

## Dialogue UI setup

No scene or prefab changes are included in this milestone. In Unity, assign the `DialogueController`, panel, speaker/body TMP text fields, Close button, and Continue button to `DialoguePresenter`. Add `DialoguePlayerControlLock` to a suitable runtime object and assign the dialogue controller, player movement component, and third-person camera controller. Assign the `PlayerInputReader` to `InteractionPromptPresenter`; its default `[{0}] {1}` format uses the configured Input System display name, so rebinding does not require a code change.
